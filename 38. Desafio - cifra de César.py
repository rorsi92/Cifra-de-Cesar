...
def cifrar_caractere(caractere, seq, chave):
    if caractere in seq:
        indice_autal = seq.index(caractere)
        novo_indice = indice_autal + chave
        # Lidar com situação onde índice está à direita da sequência
        while novo_indice >= len(seq):
            novo_indice = novo_indice - len(seq)
        # Lidar com situação onde índice está à esquerda da sequência
        while novo_indice < 0:
            novo_indice = novo_indice + len(seq)
